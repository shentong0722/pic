...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请到 https://hijk.art 查看最新内容。证书问题在我们的教程里也已经有解决方案了"
            exit 1
        }
        CERT_FILE="/usr/local/etc/xray/${DOMAIN}.pem"
        KEY_FILE="/usr/local/etc/xray/${DOMAIN}.key"
...
menu() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: ST        （备份稿）                                   #"
    echo -e "# ${GREEN}原作者网址${PLAIN}: https://hijk.art                                    #"
    echo -e "# ${GREEN}原作者论坛${PLAIN}: https://hijk.club                                   #"
    echo "#############################################################"
